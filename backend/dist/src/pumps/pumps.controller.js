"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PumpsController = void 0;
const common_1 = require("@nestjs/common");
const pumps_service_1 = require("./pumps.service");
const create_pump_data_dto_1 = require("./dto/create-pump-data.dto");
const api_key_guard_1 = require("../auth/api-key.guard");
const jwt_auth_guard_1 = require("../auth/jwt-auth.guard");
let PumpsController = class PumpsController {
    pumpsService;
    constructor(pumpsService) {
        this.pumpsService = pumpsService;
    }
    create(createPumpDataDto) {
        return this.pumpsService.createPumpData(createPumpDataDto);
    }
    findAll() {
        return this.pumpsService.getAllPumps();
    }
    findOne(pumpId) {
        return this.pumpsService.getPumpById(pumpId);
    }
    getPumpData(pumpId, limit) {
        return this.pumpsService.getPumpData(pumpId, limit ? parseInt(limit) : 100);
    }
};
exports.PumpsController = PumpsController;
__decorate([
    (0, common_1.Post)('data'),
    (0, common_1.UseGuards)(api_key_guard_1.ApiKeyGuard),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_pump_data_dto_1.CreatePumpDataDto]),
    __metadata("design:returntype", void 0)
], PumpsController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], PumpsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':pumpId'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, common_1.Param)('pumpId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], PumpsController.prototype, "findOne", null);
__decorate([
    (0, common_1.Get)(':pumpId/data'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __param(0, (0, common_1.Param)('pumpId')),
    __param(1, (0, common_1.Query)('limit')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], PumpsController.prototype, "getPumpData", null);
exports.PumpsController = PumpsController = __decorate([
    (0, common_1.Controller)('pumps'),
    __metadata("design:paramtypes", [pumps_service_1.PumpsService])
], PumpsController);
//# sourceMappingURL=pumps.controller.js.map